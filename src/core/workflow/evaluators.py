...
def check_compilation_condition(state: DomainState, policies: Policies) -> str:
    """Retorna: 'success', 'syntax_error' ou 'max_step_error'."""
...
def check_ats_condition(state: DomainState, policies: Policies) -> str:
    """Retorna: 'approved', 'max_retries', 'unfixable_gaps' ou 'needs_refinement'."""
    ats = state.ats_report
    if not ats:
        return "needs_refinement"

    # 1. Aprovado se score >= alvo e sem requisitos obrigatórios ausentes
    if not ats.hard_fail and ats.score >= policies.target_ats_score:
        return "approved"

    # 2. Atingiu teto máximo de retries
    if state.iteration >= policies.max_ats_retries:
        return "max_retries"

    # 3. Short-Circuit: se todas as skills obrigatórias ausentes forem GAPS REAIS comprovados
    if ats.missing_required:
        ...
        for missing_item in ats.missing_required:
            options = [_normalize_term(opt) for opt in missing_item.split(" OU ")]
            # Só é incorrigível se TODAS as opções da cláusula OU forem gaps reais
